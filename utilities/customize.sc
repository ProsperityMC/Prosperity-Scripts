...
command_model_set(model) -> (
    ...
    if(keys(aliases(get_player_holding() : 0)) ~ model == null,
        print(format('r That is not a valid model'));

        return(true);
    );

    item = get_player_holding() : 0;
    model_id = global_aliases : item : model;
    data = parse_nbt(get_player_holding() : 2);
...
command_lore_set(text) -> (
    ...
    held_item_name = get_player_holding() : 0;
    data = parse_nbt(get_player_holding() : 2);

    if(data : 'display' == null, 
        put(data, 'display', {});
    );  
    if(data : 'display' : 'Lore' == null, 
        data : 'display' += 'Lore';
        data : 'display' : 'Lore' = [];
    );

    data : 'display' : 'Lore' += str('{"text":"%s"}', text);
    replace_held_item(data);

    sound('minecraft:ui.cartography_table.take_result', pos(player()), 100);
    print(
        'Set the lore of your ' 
        + format(str('wi %s to ', item_display_name(held_item_name)))
...
command_name_set(text) -> (
    ...
    if(data : 'display' : 'Name' == null, 
        data : 'display' += 'Name';
    );

    data : 'display' : 'Name' = str('{"text":"%s"}', text);
    replace_held_item(data);
...
// Literally deletes the custom name property from the held item and thus resets
// the item's name.
command_name_clear() -> (
    if(error_hand_empty(), return());
...
// Returns the item data for the player's currently held item. If the no item is
// held, returns an empty map `{}`.
get_player_holding() -> (
...
// Replaces the player's held item with the NBT data formatted as a map object.
replace_held_item(nbt_as_map) -> (
    inventory_set(
        player(),
        player() ~ 'selected_slot',
        get_player_holding() : 1,
        get_player_holding() : 0,
        encode_nbt(nbt_as_map),
    );
);

// Returns `true` when the player's hand is empty. Prints an error message to 
// the chat.
error_hand_empty() -> (
    if(get_player_holding() == null,
...
__config() -> {
    ...
    'arguments' -> {
        'model' -> {
            'type' -> 'text',
                'suggester' -> _(arg) -> keys(aliases(get_player_holding() : 0)),
            },
        'text' -> {
            'type' -> 'text',
            'suggest' -> [],
